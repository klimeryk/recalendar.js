...
### For developers
 ...
 - Language names live **only** in `en/app.json`. In your own `app.json`, the `language` section must contain just `label`, `full` and `partial` - translate those three and delete every `language.<code>` entry that came along when you copied the `en` folder. Every language falls back to the English file for the names themselves, so they stay identical everywhere and a new language only has to be added in one place.
