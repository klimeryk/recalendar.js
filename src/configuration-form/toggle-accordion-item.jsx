...
function ToggleAccordionItem({ children, id, onToggle, t, title, toggledOn }) {
  const label = toggledOn ? t('configuration.toggle-form.enabled') : t('configuration.toggle-form.disabled');
  return (
    <Accordion.Item eventKey={id}>
      <Accordion.Header>
        <Stack direction="horizontal" className="w-100">
          {title}
          <Badge bg={toggledOn ? 'success' : 'secondary'} className="ms-auto me-3">
            {label}
          </Badge>
        </Stack>
      </Accordion.Header>
