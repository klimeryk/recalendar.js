...
export function getDotDiameter(lineHeightPixels) {
  const scale = toPositiveNumber(lineHeightPixels, DEFAULT_LINE_HEIGHT_PIXELS) / DEFAULT_LINE_HEIGHT_PIXELS;
  return Math.min(Math.max(DOT_DIAMETER * scale, MIN_DOT_DIAMETER), MAX_DOT_DIAMETER);
}

export function getLineSpacing(lineHeightPixels, lineSpacingPixels) {
  return toPositiveNumber(lineSpacingPixels, toPositiveNumber(lineHeightPixels, DEFAULT_LINE_HEIGHT_PIXELS));
}

export function getLineStroke({ lineStyle, lineHeightPixels, lineSpacingPixels, lineOpacity }) {
  const style = normalizeLineStyle(lineStyle);
  const opacity = Number.isFinite(Number(lineOpacity)) ? Number(lineOpacity) : DEFAULT_LINE_OPACITY;
  ...
  if (style === GRID) {
    const height = toPositiveNumber(lineHeightPixels, DEFAULT_LINE_HEIGHT_PIXELS);
    return {
      color,
      height,
