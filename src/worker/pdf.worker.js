...
function encodeConfig(data) {
  const dataWithVersion = Object.assign({ version: CONFIG_CURRENT_VERSION }, data);
  return 'data:text/plain;base64,' + utf8ToBase64(JSON.stringify(dataWithVersion));
}
...
self.onmessage = ({ data }) => {
  ...
  const document = React.createElement(RecalendarPdf, { isPreview, config }, null);
  pdf(document, {
    attachments: [
      {
